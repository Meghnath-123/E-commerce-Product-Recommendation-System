...
def content_based_recommendations(train_data, item_name, top_n=10):
    if item_name is None:
        logging.debug("No item name provided.")
        return pd.DataFrame()
    
    # Check if the item name exists in the training data
    if item_name not in train_data['Name'].values:
        logging.debug(f"Item '{item_name}' not found in the training data.")
        return pd.DataFrame()
...
@app.route("/main")
def main():
    content_based_rec = content_based_recommendations(train_data, None, top_n=10)
    random_product_image_urls = [random.choice(random_image_urls) for _ in range(len(trending_products))]
    price = [40, 50, 60, 70, 100, 122, 106, 50, 30, 50]

    message = "No recommendations available for this product." if content_based_rec.empty else None

    return render_template('main.html', message=message, content_based_rec=content_based_rec, truncate=truncate,
                           random_product_image_urls=random_product_image_urls,
                           random_price=random.choice(price))
...
def recommendations():
    if request.method == 'POST':
        ...
        try:
            ...
            nbr = 10  # Default to 10 if conversion fails
        ...
        if content_based_rec.empty:
            ...
            logging.debug(content_based_rec)
            logging.debug(random_product_image_urls)
